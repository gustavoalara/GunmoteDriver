...
echo %BASEDIR%\tools\devcon\i386\devcon.exe install gunmotep1.inf gunmote\gunmotep1 > bin\install_driver.bat

:: create bat file for easier testing
echo %BASEDIR%\tools\devcon\i386\devcon.exe remove gunmote\gunmotep1 > bin\test.bat
echo cd .. >> bin\test.bat
echo cmd /c buildme.bat >> bin\test.bat
echo cd bin >> bin\test.bat
echo cmd /c install_driver.bat >> bin\test.bat
echo echo After keypress, launching testgunmotep1 >> bin\test.bat
